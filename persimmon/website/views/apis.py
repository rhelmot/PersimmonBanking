...
def approve_transaction_page(request, tid):
    ...
    if form.is_valid():
        verification_code = make_verification_code(
            ...
            user.username,
            str(tid))
        form.fields['email_verification'] = forms.CharField(widget=forms.TextInput(
    ...
    if form_email.is_valid() and form_email.cleaned_data['email'] != original_email:
        if not editing_self:
            ...
            verification_code_2 = make_verification_code(
                ...
                form_email.cleaned_data['email'],
                form_email.cleaned_data['email'])
            form_email.fields['email_verification_1'] = forms.CharField(widget=forms.TextInput(
                attrs={'class': 'use-otpkeyboard-input'}))
            form_email.fields['email_verification_2'] = forms.CharField(widget=forms.TextInput(
                attrs={'class': 'use-otpkeyboard-input'}))
    ...
    if form_phone.is_valid() and form_phone.cleaned_data['phone'] != original_phone:
        if not editing_self:
            ...
            verification_code_2 = make_verification_code(
                ...
                form_phone.cleaned_data['phone'],
                form_phone.cleaned_data['phone'])
            form_phone.fields['phone_verification_1'] = forms.CharField(widget=forms.TextInput(
                attrs={'class': 'use-otpkeyboard-input'}))
            form_phone.fields['phone_verification_2'] = forms.CharField(widget=forms.TextInput(
                attrs={'class': 'use-otpkeyboard-input'}))
