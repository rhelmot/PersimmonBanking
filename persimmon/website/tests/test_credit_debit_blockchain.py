...
class TestCreditDebitWorkFlow(TestCase):
    ...
    def test_workfolow(self):
        make_user('user')
        client_new_user = Client()
        self.assertTrue(client_new_user.login(username='user', password='password'))
        make_user('admin', employee_level=EmployeeLevel.ADMIN)
        client_new_admin = Client()
        self.assertTrue(client_new_admin.login(username='admin', password='password'))
        req1 = client_new_user.post(
        ...
        req = client_new_user.post(reverse(views.credit_debit_funds), content_type="application/json",
        ...
        req = client_new_admin.post(reverse(views.get_pending_transactions), content_type="application/json",
                                data={"account_id": req_data['account']})
        self.assertEqual(req.status_code, 200)
        ...
        req = client_new_admin.post(reverse(views.approve_credit_debit_funds), content_type="application/json",
